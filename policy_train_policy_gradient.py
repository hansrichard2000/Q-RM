...
from src.ppo.trainer import ParallelPolicyGradientTrainerForCausalLM, ParallelGRPOTrainerForCausalLM, \
    ParallelPolicyGradientConvexTrainerForCausalLM
from src.utils import masked_mean, json_load, print_current_func_args
...
def train_policy_gradient(
    ...
    optimizer = DataParallelOptimizer(torch.optim.Adam(policy.parameters(), lr=lr))
    ...
    for global_epoch in range(begin_global_epoch, epochs):
        for local_epoch in range(begin_local_epoch, local_epochs):
            ...
            if use_last_token_reward:
                ...
                print(f"Average Rewards: {masked_mean(verifier_rollout_buffer.scores, policy_rollout_buffer.action_masks)}")
