...
from src.utils import logits_assignment, powmax, log1m_softmax, proxy_neg_distribution
...
class ParallelPolicyGradientConvexTrainerForCausalLM(ParallelTrainer):
    ...
    def forward(self, rollout_data: RolloutBufferSample):
        ...
        loss_pos = - torch.mean(rewards[pos_reward_masks] * action_logprobs[pos_reward_masks]).nan_to_num(0.0)

        loss_neg = 0.0
        if self.delta > 0:
            targets = proxy_neg_distribution(logits[~pos_reward_masks], actions[~pos_reward_masks], self.delta)
            loss_neg = - torch.mean(rewards[~pos_reward_masks] * self.criterion.forward(
                torch.log_softmax(logits[~pos_reward_masks], dim=-1), target=targets
            ).sum(-1)).nan_to_num(0.0)
        ...
        if self.step % 100 == 0:
            print(f"Positive Reward Loss: {loss_pos} | Negative Reward Loss: {loss_neg}")
...
class ParallelPolicyGradientTrainerWithKLDivForCausalLM(ParallelTrainer):
    ...
    def modified_kl_loss(self, logits, rewards, actions, action_masks) -> torch.Tensor:
        ...
        loss = torch.masked_select(
            self.criterion.forward(torch.log_softmax(logits, dim=-1), labels).sum(-1).view(-1),
            action_masks.view(-1)
        )
        rewards = torch.masked_select(rewards.view(-1), action_masks.view(-1))
