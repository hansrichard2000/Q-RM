...
class CheckpointForQwen(Checkpoint):
    ...
    #
    # @classmethod
    # def duplicate_kv(cls, ckpt_dir: str, origin_kv_heads: int, target_kv_heads: int):
    #     from safetensors.torch import save_file
    #     assert target_kv_heads % origin_kv_heads == 0
    #     ckpt_files = sorted(Path(ckpt_dir).glob("model*.safetensors"))
    #     for ckpt_file in ckpt_files:
    #         state_dict = OrderedDict()
    #         ckpt_file = str(ckpt_file)
    #         print(f"Loading {ckpt_file} ...")
    ...
    #             for k in reader.keys():
    #                 state_dict[k] = reader.get_tensor(k)
    #                 if ".self_attn.k_proj." in k or ".self_attn.v_proj." in k:
    #                     state_dict[k] = torch.repeat_interleave(
    #                         torch.reshape(
    #                             state_dict[k],
    #                             shape=[state_dict[k].shape[0] // origin_kv_heads, origin_kv_heads, -1]
    #                         ), repeats=target_kv_heads // origin_kv_heads, dim=1
    #                     ).reshape(state_dict[k].shape[0] // origin_kv_heads * target_kv_heads, -1).squeeze()
